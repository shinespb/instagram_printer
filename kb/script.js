setTimeout(function(){
	...
	function virtualKeyboardChromeExtension_click(key, skip) {
		if (top != self) {
			sendRequest({method: "clickFromIframe", key: key, skip: skip, frame: this.frameElement.id });
		} else {
			if (key != "Close") {
	...
	function setting_load(key, callback) {
		sendRequest({method: "getLocalStorage", key: key}, function(response) {
		  	eval(callback)(response.data);
		});
	}

	function setting_reloadKeyboardToggle(openState) {
		sendRequest({method: "getLocalStorage", key: "keyboardEnabled"}, function(response) {
	...
	function setting_set(key, value) {
		sendRequest({method: "setLocalStorage", key: key, value: value}, function(response) {});
	}
	...
	function virtualKeyboardChromeExtension_open_part2(pos) {
		...
		sendRequest({method: "getSmallKeyboardCoords"}, function(response) {
	...
	function virtualKeyboardChromeExtension_open(posY, posX, force) {
		if (top != self) {
	        ...
	        sendRequest();*/
			sendRequest({method: "openFromIframe", posY: posY, posX: posX, force: force, frame: this.frameElement.id, elem: virtualKeyboardChromeExtensionClickedElem.id });
		} else {
			...
			if ((virtualKeyboardChromeExtensionKeyboardEnabled == "demand") && (force == undefined) && (virtualKeyboardChromeExtensionClickedElem.id != "virtualKeyboardChromeExtensionUrlBarTextBox")) {
				...
				if (virtualKeyboardChromeExtensionKeyboardLoaded1 != virtualKeyboardChromeExtensionKeyboardLayout1Setting) {
					...
					xmlhttp.open("GET",("kb/keyboard_"+virtualKeyboardChromeExtensionKeyboardLayout1Setting+".html"),true);
					xmlhttp.send();
	...
	function xk_settings_load_main(response) {
		console.log('xk_settings_load_main', response);
	...
	function init_virtualKeyboardChromeExtension(firstTime) {
		console.log('init_virtual', firstTime);
		if (firstTime) {
			if (top == self) {
				if (virtualKeyboardChromeExtensionTouchEvents == undefined) {			
					if ((document.getElementById('virtualKeyboardChromeExtension').getAttribute("_state") != "open") || (virtualKeyboardChromeExtensionRequestRefresh)) {
						sendRequest({method: "loadKeyboardSettings"}, xk_settings_load_main);
					} 
				} else {
				...
				if (virtualKeyboardChromeExtensionTouchEvents == undefined) {			
					sendRequest({method: "loadKeyboardSettings"}, function(response) {
						virtualKeyboardChromeExtensionTouchEvents = response.touchEvents;
						if (virtualKeyboardChromeExtensionTouchEvents == undefined) { virtualKeyboardChromeExtensionTouchEvents = "false"; }
						init_virtualKeyboardChromeExtension(true);
					});
		...
		if (virtualKeyboardChromeExtensionTouchEvents != undefined) {
			...
			if (firstTime) {
				...
				if (top == self) {
					...
					delete m;
					if (true || document.getElementById("settingsButton") != undefined) {
						sendRequest({method: "getLocalStorage", key: "keyboardLayoutsList"}, function(response) {
	...
	if (top == self) {
	 ...
	 //            virtualKeyboardChromeExtension_generate_onchange();
		...
		xhr.open("GET", "kb/keyboard.html", true);
	...
	function vk_ajax_load_main() {
	  if (xhr.readyState == 4) {
		...
		sendRequest({method: "initLoadKeyboardSettings"}, function(response) {
